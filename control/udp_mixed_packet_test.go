...
func GenerateTestPackets() []TestPacket {
	...
	// Optimized: Port heuristic no longer forces ordered ingress
	// ShouldUseOrderedIngress is now false (port heuristic removed)
	// ShouldAttemptSniff only checks IsQuicInitial, not IsLikelyQuicData
	for i := 0; i < 3; i++ {
		pkt := TestPacket{
			Type:        PacketTypeDTLS,
		...
		pkt.Expected.IsQuicInitial = false
		pkt.Expected.ShouldUseOrderedIngress = false // Optimized: no longer uses IsLikelyQuicData
