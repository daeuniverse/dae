...
func TestDnsController_CloseWaitsForWorkersInParallel(t *testing.T) {
	c := &DnsController{
		janitorStop:   make(chan struct{}),
		janitorDone:   make(chan struct{}),
		evictorDone:   make(chan struct{}),
		bpfUpdateStop: make(chan struct{}),
		bpfUpdateDone: make(chan struct{}),
	}

	go func() {
		<-c.bpfUpdateStop
		time.Sleep(80 * time.Millisecond)
		close(c.bpfUpdateDone)
	}()
	go func() {
		<-c.janitorStop
		time.Sleep(80 * time.Millisecond)
	...
	require.Less(t, elapsed, 200*time.Millisecond, "close should wait for shutdown workers in parallel")
}

func TestDnsController_CloseSuppressesRemoveCallbacks(t *testing.T) {
	var removed atomic.Int32
	c := &DnsController{
		cacheRemoveCallback: func(cache *DnsCache) error {
			removed.Add(1)
	...
	c.onDnsCacheEvicted(&DnsCache{})
	require.Zero(t, removed.Load(), "shutdown should suppress cache remove side effects")
}

func TestDnsController_CloseFastDoesNotWaitForWorkers(t *testing.T) {
	c := &DnsController{
		janitorStop:   make(chan struct{}),
		janitorDone:   make(chan struct{}),
		evictorDone:   make(chan struct{}),
	...
	require.NoError(t, err)
	require.Less(t, elapsed, 20*time.Millisecond, "fast close should not wait for Go-side workers")
}
